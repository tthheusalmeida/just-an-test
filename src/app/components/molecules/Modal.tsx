...
export default function Modal({
  ...
  return createPortal(
    <div className="fixed inset-0 flex z-[99] items-center justify-center w-full">
      ...
      >
        {title && (
          <div className="flex items-center gap-2 absolute w-full justify-start max-w-[var(--page-width)] top-0 mt-5 ml-8">
            <div className="bg-transparent text-xl text-darkTurquoise-700">
              {title}
            </div>
          </div>
        )}
        <div className="flex absolute w-full justify-end max-w-[var(--page-width)] top-0">
          <Button
            onClick={onClose}
            variant="secondary"
            className="!w-fit bg-transparent mt-1 mr-4 text-darkTurquoise-700"
            disableAnimation
          >
